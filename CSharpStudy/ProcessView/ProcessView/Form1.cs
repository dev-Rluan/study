...
namespace ProcessView
{
    public partial class Form1 : Form
    {
        ...
        private void ProcessView()
        {
            try
            {
                this.lvView.Items.Clear(); cp = 0; foreach (var proc in Process.GetProcesses())
                {
                    string[] str; try
                    ...
                    var lvt = new ListViewItem(strArray); this.lvView.Items.Add(lvt); cp++;
                }
            }
            catch { }
        ...
        { 
            MemNum = MemNum / 1024; return String.Format("{0:N}", MemNum) + " KB"; 
        }
            ...
            try
            {
                ...
                if (!(tProcess == null))
                {
                    var dlr = MessageBox.Show(this.lvView.SelectedItems[0].SubItems[0].Text +
                                        " 프로세스를 끝내시겠습니까?", "알림", MessageBoxButtons.YesNo, MessageBoxIcon.Warning); 
                ...
                else
                {
                  ...
                  "프로세스는 존재하지 않습니다", "알림", MessageBoxButtons.OK, MessageBoxIcon.Error); ProcessView();
                }
            }
